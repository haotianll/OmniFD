...
class Resampler(BaseModule):
    def __init__(self,
        ...
        self.sampler = Resampling(dim=dim, *args, **kwargs)
    ...
    def forward(self, inputs, data_samples=None, **kwargs):
        ...
        for i, (proj, input) in enumerate(zip(self.proj, reversed(inputs))):
            ...
            feat = feat.contiguous().permute(0, 2, 1, 3, 4).reshape(B, 1, -1, C)  # B C T H W -> B T H W C ->  B 1 THW C
            x.append(feat)

        x = torch.cat(x, dim=-2)

        x = self.sampler(x)  # B, 1, K, C

        x = x.squeeze(1)  # B, K, C
        x = x.contiguous().permute(0, 2, 1).contiguous()  # B K C -> B C K
        return inputs, x
