...
class SwinTransformer3D(nn.Module):
    ...
    def forward(
        ...
        if 'all' in out_feat_keys:
            stage_outputs[-1] = self._apply_norm(stage_outputs[-1])
            return stage_outputs
        elif out_feat_keys is not None and len(out_feat_keys) > 0:
...
class SwinTransformerOmni(nn.Module):
    def __init__(self,
                 pretrained2d=False,
                 pretrained=None,
                 patch_size=(2, 4, 4),
        ...
        if checkpoint_name is not None:
            self.load_checkpoint(checkpoint_name=checkpoint_name)
    ...
    def load_checkpoint(
        ...
        if pretrained:
            path = CHECKPOINT_PATHS[checkpoint_name]
            # All models are loaded onto CPU by default
            checkpoint = load_state_dict_from_url(path, progress=progress, map_location="cpu")
            missing_keys, unexpected_keys = self.trunk.load_state_dict(checkpoint["trunk"], strict=False)
    ...
    def forward(self, x: torch.Tensor, input_type: Optional[str] = None):
        ...
        Returns:
            preds: tensor of shape (1, num_classes)
        """
        features = self.trunk(x, out_feat_keys=self.out_feat_keys, use_checkpoint=self.with_cp)
        return features
